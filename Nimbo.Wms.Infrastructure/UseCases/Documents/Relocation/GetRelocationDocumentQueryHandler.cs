...
[PublicAPI]
public class GetRelocationDocumentQueryHandler : IRequestHandler<GetRelocationDocumentQuery, Result<RelocationDocumentDto>>
{
    ...
    public async Task<Result<RelocationDocumentDto>> Handle(GetRelocationDocumentQuery request, CancellationToken ct)
    {
        ...
        if (document is null)
            return Error.NotFound("document.notfound", $"Relocation document with ID '{request.Id}' not found.");
